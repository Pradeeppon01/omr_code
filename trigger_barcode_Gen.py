...
def record_placeholder_data(data,current_record_id,cursor):
    ...
    try:
        insert_query = f"INSERT INTO {PLACEHOLDER_RECORD_TABLE} (sno,slno,degreeWithBranch,candidateName,registerNumber,examDateAndSession,examCentreCode,subjectCode,subjectType,collegeCode,studentSemester,subjectTitle,questionCode,barCode1,barCode2,barCode3,barCode4,barCode5,barCode6,barCode7,barCode8,barCode9,barCode10,qrCode1,qrCode2,qrCode3,qrCode4,qrCode5,qrCode6,qrCode7,qrCode8,qrCode9,qrCode10) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)" 
        insert_values = (current_record_id,data.get("SL.NO"),data.get("DEGREE_WITH_BRANCH"),data.get("Name"),data.get("Regno"),data.get("EXAM_DATE_&_SESSION"),data.get("EXAM_CENTER_CODE"),data.get("SubCode"),data.get("stusub_type"),data.get("CollegeCode"),data.get("stu_semester"),data.get("SubName"),data.get("QUESTION_CODE"),data.get("BARCODE1"),data.get("BARCODE2"),data.get("BARCODE3"),data.get("BARCODE4"),data.get("BARCODE5"),data.get("BARCODE6"),data.get("BARCODE7"),data.get("BARCODE8"),data.get("BARCODE9"),data.get("BARCODE10"),data.get("QRCODE1"),data.get("QRCODE2"),data.get("QRCODE3"),data.get("QRCODE4"),data.get("QRCODE5"),data.get("QRCODE6"),data.get("QRCODE7"),data.get("QRCODE8"),data.get("QRCODE9"),data.get("QRCODE10"))
        ...
        if db_response.get("status") == "success":
            ...
            for index,row in enumerate(csv_reader):
                ...
                while barcode_generated_count < BARCODE_GENERATION_COUNT:
                    ...
                    qrcode_label = f"QRCODE{barcode_generated_count+1}"
                    print(barcode_label, " : ",barcode)
                    qrcode = int(barcode)+int(QRCODE_ADDITION_CONSTANT)
                    row[barcode_label]=barcode
                    row[qrcode_label]=qrcode
